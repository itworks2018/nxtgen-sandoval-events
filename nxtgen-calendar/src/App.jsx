...
import { Plus, Pencil, Trash2, X, ChevronLeft, ChevronRight, MapPin, Users, Clock, StickyNote, ShieldCheck, Eye, CalendarDays, Loader2, Megaphone, AlertTriangle, Link2, ExternalLink, Grid3x3, List } from "lucide-react";
...
export default function App() {
  ...
  // Get all events for current month, sorted by date/time
    ...
    <div style={styles.app}>
      ...
      <div style={styles.navRow}>
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
          <button 
            className="nxg-btn" 
            style={{ ...styles.todayBtn, ...(viewMode === "grid" ? {} : { opacity: 0.6 }) }}
          ...
          <button 
            className="nxg-btn" 
            style={{ ...styles.todayBtn, ...(viewMode === "list" ? {} : { opacity: 0.6 }) }}
        ...
        {viewMode === "grid" ? (
          ...
          <div style={styles.listView}>
            {loading ? (
              ...
              <p style={styles.emptyText}>No events in {MONTH_NAMES[month]} {year}.</p>
            ) : (
              <div style={styles.listContainer} className="nxg-scroll">
                {getMonthEvents().map((ev) => (
                  <div key={ev.id} style={{ ...styles.listEventCard, borderLeftColor: venueColor(ev.venue) }}>
                    <div style={styles.listEventHead}>
                      <div>
                        <h4 style={styles.listEventName}>{ev.name}</h4>
                        <div style={styles.listEventMeta}>
                          {ev.startDate && <span>{formatDateHeading(ev.startDate)}</span>}
                          {ev.startTime && <span style={{ marginLeft: 12 }}>• {formatTime12(ev.startTime)}{ev.endTime ? ` - ${formatTime12(ev.endTime)}` : ""}</span>}
                        </div>
                      </div>
                      {teacherMode && (
                    ...
                    <div style={styles.listEventDetails}>
                      {ev.venue && <div style={styles.listDetailRow}><MapPin size={13} /> <span>{ev.venue}</span></div>}
                      {ev.participants && <div style={styles.listDetailRow}><Users size={13} /> <span>{ev.participants}</span></div>}
                      {ev.notes && <div style={styles.listDetailRow}><StickyNote size={13} /> <span>{ev.notes}</span></div>}
                    </div>
                  </div>
                ))}
              </div>
